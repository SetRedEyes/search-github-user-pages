...
export const GithubState = ({ children }) => {
  ...
  const search = async (value) => {
    ...
    dispatch({
      type: SEARCH_USERS,
      payload: response.data.items /* данные пришедшие с гитхаба */
    })
  }

  const getUser = useCallback(async (names) => {
    setLoading()

    const response = await axios.get(withCreds(`https://api.github.com/users/${names}?`))
  ...
  return (
    <GithubContext.Provider value={{ setLoading, search, getUser, getRepos, clearUsers, user, users, repos, loading }}>
      {children}
